...
\restrict NveyLGIMUNuES4wORzC5B4KaPQkO7m6H2pbh4iyQ3osNkW8SEY9eh80me3Z5mw5
...
COPY public.alert_logs (id, alert_rule_id, prescription_id, patient_id, triggered_by, context, action_taken, override_reason, created_at) FROM stdin;
\.
...
1	High Opioid Dosage	Alert when morphine equivalent dosage exceeds threshold	{"type": "morphine_equiv", "threshold": 50, "threshold_unit": "mg/day", "check_patient_history": true}	high	t	2024-01-01 00:00:00-08
2	Opioid-Benzodiazepine Interaction	Alert when opioid and benzodiazepine co-prescribed	{"type": "drug_interaction", "drug_classes": ["opioid", "benzodiazepine"], "dea_monitored": true, "severity_interaction": "critical"}	critical	t	2024-01-01 00:15:00-08
3	PDMP Red Flag	Alert based on PDMP query results showing high-risk patterns	{"type": "pdmp_flag", "days_window": 30, "controlled_substances": ["II", "III", "IV"], "consecutive_prescribers": 4}	high	t	2024-01-01 00:30:00-08
4	Allergy Conflict	Alert when medication conflicts with documented allergies	{"type": "allergy_conflict", "severity": "critical", "check_cross_reactivity": true}	critical	t	2024-01-02 00:00:00-08
5	Duplicate Therapy	Alert when patient prescribed multiple medications in same class	{"type": "duplicate_therapy", "drug_classes": ["statin", "beta_blocker", "ace_inhibitor", "antibiotic"], "same_class_limit": 1}	medium	t	2024-01-02 00:15:00-08
6	Age-Based Dosing	Alert for inappropriate dosing based on patient age	{"type": "age_based_dosing", "geriatric_threshold": 65, "pediatric_threshold": 18, "check_beers_criteria": true}	high	t	2024-01-02 00:30:00-08
7	Renal Impairment Dosing	Alert when medication requires dose adjustment for renal function	{"type": "renal_dosing", "medications_affected": ["metformin", "gabapentin", "nsaid"], "creatinine_clearance_threshold": 30}	high	t	2024-01-03 00:00:00-08
8	Hepatic Impairment Dosing	Alert when medication requires dose adjustment for hepatic function	{"type": "hepatic_dosing", "alt_threshold": 100, "ast_threshold": 100, "bilirubin_threshold": 3}	medium	t	2024-01-03 00:15:00-08
9	Pregnancy Contraindication	Alert when medication is contraindicated in pregnancy	{"type": "pregnancy_contraindication", "category_c_alert": true, "category_d_critical": true, "category_x_emergency": true}	critical	t	2024-01-03 00:30:00-08
10	FERPA Student Record Access	Alert on access to student health records	{"type": "ferpa_student_access", "log_all_access": true, "require_justification": true}	medium	t	2024-01-04 00:00:00-08
\.
...
COPY public.audit_logs (id, entity_type, entity_id, user_id, action, payload, ip_address, user_agent, created_at) FROM stdin;
\.
...
COPY public.dir_fees (id, claim_id, amount, reason, recorded_at) FROM stdin;
\.
...
d35c1031-6c11-4c2e-b571-ef671942d32a	test	test	test@gmail.com	6	t	2025-11-14 20:06:20.362-08	\N
...
SELECT pg_catalog.setval('public.alert_logs_id_seq', 21, true);
...
SELECT pg_catalog.setval('public.dir_fees_id_seq', 5, true);
...
SELECT pg_catalog.setval('public.prescription_workflow_logs_id_seq', 1, false);
...
ALTER TABLE ONLY public.dir_fees
    ADD CONSTRAINT dir_fees_claim_id_fkey FOREIGN KEY (claim_id) REFERENCES public.claims(id);
...
\unrestrict NveyLGIMUNuES4wORzC5B4KaPQkO7m6H2pbh4iyQ3osNkW8SEY9eh80me3Z5mw5
