...
public class FileUtils {
     ...
     */
    public static void writeListOfStringToFile(File filename, List<String> list)
            throws IOException {
        if (null == filename) {
            throw new IllegalArgumentException("Input file required!");
        }
    ...
    public static String readFileFromResource2String(String resourceName, Charset encoding)
        ...
        try {
            encoded =
                    Files.readAllBytes(
                            Paths.get(ClassLoader.getSystemResource(resourceName).toURI()));
